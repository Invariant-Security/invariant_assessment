...
def test_parses_name_image_and_id_triples(monkeypatch):
    monkeypatch.setattr(
        subprocess,
        "run",
        _fake_run(
            "web-1|nginx:1.27-alpine|a1b2c3d4e5f6\n"
            "api-1|myorg/api:latest|f6e5d4c3b2a1\n"
        ),
    )
    ...
    assert result == [
        {"name": "web-1", "image": "nginx:1.27-alpine", "id": "a1b2c3d4e5f6"},
        {"name": "api-1", "image": "myorg/api:latest", "id": "f6e5d4c3b2a1"},
    ]
...
def test_ignores_blank_lines(monkeypatch):
    ...
    assert list_docker_containers() == [{"name": "web-1", "image": "nginx:1.27-alpine", "id": "a1b2c3d4e5f6"}]
